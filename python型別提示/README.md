...
網頁版的 JupyterLab (4.0+) 與 Jupyter Notebook (7.0+) 已經原生內建 LSP (Language Server Protocol) 支援。您可以透過安裝 **basedpyright** 來獲得與 VS Code 同等強大的靜態型別分析與自動補齊功能：

1. **安裝 LSP 擴充套件與 basedpyright**：
   在您執行 Jupyter 的 Python 虛擬環境中，執行以下指令安裝：
   ```bash
   # 安裝 Jupyter LSP 整合套件
   pip install jupyterlab-lsp

   # 安裝 basedpyright 靜態分析伺服器
   pip install basedpyright
   ```
2. **啟用與自動偵測**：
   - 重新啟動 Jupyter 伺服器，系統會自動在環境中偵測 `basedpyright` 並將其啟用為 Python 的語言伺服器。
